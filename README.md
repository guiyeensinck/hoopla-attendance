...
Con la variable `ANTHROPIC_API_KEY` seteada, cuando un DM no matchea ningún comando ni es una imputación, el bot responde con Claude (modelo configurable vía `IA_MODEL`, default Haiku) en vez de mostrar el menú. Recibe el contexto real de la persona (marcaciones de hoy y del último día hábil con flags de auto-cierre, horas netas, imputaciones, saldo semanal/mensual y catálogo de proyectos), así puede explicar cálculos ("¿por qué dice que trabajé 6hs?") y guiar a los comandos. Es solo lectura: no modifica registros. Usa el historial reciente del DM para mantener el hilo. Sin la variable o ante cualquier error de la API, cae al menú de siempre.
